...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Прошивка вузла КОРОЛЕВА (LoRa RX -> Reflex OTA -> SIM7070G -> Rails)
...
uint32_t current_sender_id = 0;         // UID Солдата (перші 4 байти)
...
int main(void)
{
  ...
  // 3. Відкриваємо вуха: Королева переходить у режим безперервного слухання
  ...
  while (1)
  {
    ...
    if (lora_rx_flag == 1) 
    {
        ...
        // =========================================================================
        // ТРАНЗИТ НА RAILS-БЕКЕНД
        // =========================================================================
        // Тільки тепер, коли ефір вільний і Солдат отримав своє, ми звертаємося до інтернету.
        ...
        // Відправляємо 16 розшифрованих байтів на сервер у Черкаси
    ...
    // Зауваження: Королева не засинає (не викликає STOP2Mode), 
    // оскільки живиться від сонячної панелі з постійним доступом до енергії.
...
// =========================================================================
// ФОРМУВАННЯ JSON ТА HTTP POST
// =========================================================================
void Send_Data_To_Rails(uint32_t device_id, uint8_t* payload, int8_t rssi)
{
    char json_body[256];
    
    // Форматуємо всі 16 байтів у суцільний hex-рядок (напр. "AABBCCDDEE...")
    // Це дозволить бекенду розпарсити TTL, статус акустики, хаос тощо.
    ...
    SIM7070_SendATCommand(json_body, 500); // Передача JSON
